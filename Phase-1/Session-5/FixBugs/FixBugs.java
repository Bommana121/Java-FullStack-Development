package fixBugs;

import java.util.ArrayList;
import java.util.Scanner;

public class FixBugs {

    public static void main(String[] args) {
       
        ArrayList<Integer> expenses = new ArrayList<Integer>();
        expenses.add(1000);
        expenses.add(2300);
        expenses.add(45000);
        expenses.add(32000);
        expenses.add(110);
       optionsSelection(expenses);

    }
    private static void optionsSelection(ArrayList<Integer> arrlist) {
    	
        String[] arr = {"1. I wish to review my expenditure",
                "2. I wish to add my expenditure",
                "3. I wish to delete my expenditure",
                "4. I wish to sort the expenditures",
                "5. I wish to search for a particular expenditure",
                "6. Close the application"
        };
       int[] arr1 = {1,2,3,4,5,6};
        int  slen = arr1.length;
        for(int i=0; i<slen;i++){
            System.out.println(arr[i]);
        }
        
        System.out.println("\nEnter your choice:\t");
        Scanner sc = new Scanner(System.in);
        int  options =  sc.nextInt();
        for(int j=1;j<=slen;j++){
            if(options==j){
                switch (options){
                    case 1:
                        System.out.println("Your expenses are listed below: \n");
                        System.out.println(arrlist+"\n");
                        optionsSelection(arrlist);
                        break;
                    case 2:
                        System.out.println("Enter the value to add your Expense: \n");
                        int value = sc.nextInt();
                        arrlist.add(value);
                        System.out.println("Your value is updated\n");
                      //  expenses.addAll(arrlist);
                        System.out.println(arrlist+"\n");
                        optionsSelection(arrlist);

                        break;
                    case 3:
                        System.out.println("You are about the delete all your expenses! \nConfirm again by selecting the same option...\n");
                        int con_choice = sc.nextInt();
                        if(con_choice==options){
                               arrlist.clear();
                            System.out.println(arrlist+"\n");
                            System.out.println("All your expenses are erased!\n");
                        } else {
                            System.out.println("Oops... try again!\n");
                        }
                        optionsSelection(arrlist);
                        break;
                    case 4:
                        sortExpenses(arrlist);
                        optionsSelection(arrlist);
                        break;
                    case 5:
                        searchExpenses(arrlist);
                        optionsSelection(arrlist);
                        break;
                    case 6:
                        closeApp();
                        break;
                    default:
                        System.out.println("Invalid choice!\n");
                        break;
                }
            }
        }

    }
    
   
  
    private static void closeApp() {
        System.out.println("Closing your application... \nThank you!\n");
            }
    private static void searchExpenses(ArrayList<Integer> arrayList) {
        System.out.println("Enter the expense you need to search:\t");
        Scanner s = new Scanner(System.in);
        int key = s.nextInt();
        System.out.println(key+" is found at index: "+linearSearch(arrayList, key));    
        System.out.println();
    }
    public static int linearSearch(ArrayList<Integer> arr, int key){    
        for(int i=0;i<arr.size();i++){    
            if(arr.get(i)==key) 
			{    
                return i;    
            }	    
        }    
        return -1;    
    }   

    private static void sortExpenses(ArrayList<Integer> arrayList)  {
        int temp;
        boolean sorted = false;

        while (!sorted) {
            sorted = true;
            for (int i = 0; i < arrayList.size()-1; i++) {
                if (arrayList.get(i).compareTo(arrayList.get(i + 1)) > 0) {
                    temp = arrayList.get(i);
                    arrayList.set(i, arrayList.get(i + 1));
                    arrayList.set(i + 1, temp);
                    sorted = false;
                }
            }
         }
        System.out.println("The sorted elements are : " +arrayList);   
        System.out.println();
    }
}
